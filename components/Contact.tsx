"use client";
import React from "react";
import Link from "next/link";

import { Raleway } from "next/font/google";
import { classNames } from "./utils/helper";
const raleway = Raleway({
    weight: "400",
    subsets: ["latin"],
})

const Contact = () => {
    return (
        <div className="min-h-screen mt-20 bg-[url('/assets/back4.png')] bg-blend-luminosity bg-no-repeat bg-left bg-cover">
            <div className="bg-gradient-to-b from-[rgba(0,0,0,0)] to-[rgba(0,0,0,0.2)] flex flex-col">
                <div className="flex flex-col gap-3 items-center justify-center text-5xl py-10 font-semibold">
                    <span className="uppercase text-transparent bg-clip-text bg-gradient-to-b from-[#0298F9] to-[#086BEA]">Connect with us</span>
                    <svg width="450" height="1" viewBox="0 0 580 1" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <line y1="0.5" x2="580" y2="0.5" stroke="url(#paint0_linear_11_3)" />
                        <defs>
                            <linearGradient id="paint0_linear_11_3" x1="622.006" y1="1" x2="-40.7939" y2="0.999999" gradientUnits="userSpaceOnUse">
                                <stop stopColor="white" stopOpacity="0" />
                                <stop offset="0.501042" stopColor="white" />
                                <stop offset="1" stopColor="white" stopOpacity="0" />
                            </linearGradient>
                        </defs>
                    </svg>
                </div>
                <div className="h-full grid grid-cols-2 items-center justify-center gap-10 pt-10">
                    <div className="flex flex-col items-end justify-start gap-10">
                        <div className="bg-gradient-to-b from-[#0298F9] to-[#086BEA] p-[1px] w-1/2">
                            <input className="bg-black text-white w-full py-6 px-5" placeholder="Name" />
                        </div>
                        <div className="bg-gradient-to-b from-[#0298F9] to-[#086BEA] p-[1px] w-1/2">
                            <input className=" bg-black text-white w-full py-6 px-5" placeholder="Email" />
                        </div>
                        <div className="bg-gradient-to-b from-[#0298F9] to-[#086BEA] p-[1px] w-1/2">
                            <input className="bg-black text-white w-full py-6 px-5" placeholder="Phone" />
                        </div>
                    </div>
                    <div className="mt-0 h-full">
                        <div className="bg-gradient-to-b from-[#0298F9] to-[#086BEA] p-[1px] w-3/4 h-full">
                            <textarea className="bg-black h-full text-white pt-2 px-5 resize-none w-full" placeholder="Message" />
                        </div>
                    </div>
                </div>
                <button className="bg-gradient-to-b from-[#0298F9] to-[#086BEA] uppercase w-1/4 self-center py-3 mt-10 text-white">Submit</button>
            </div>
        </div>
    )
}

export default Contact;